...
if [ -d opus_src ]; then (cd opus_src && git fetch); else (mkdir -p opus_src && git clone https://chromium.googlesource.com/chromium/deps/opus.git opus_src);fi; (cd opus_src && git reset --hard && git clean -xdf && git checkout cae6961 && patch -p1 -i ../opus_private_export.patch)
